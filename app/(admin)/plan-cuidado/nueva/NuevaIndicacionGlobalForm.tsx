...
const INPUT = 'w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2'
const SELECT = `${INPUT} bg-white`

const hoy = () => new Date().toISOString().split('T')[0]

export default function NuevaIndicacionGlobalForm({ pacientes }: { pacientes: Paciente[] }) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/plan-cuidado" className="text-gray-400 hover:text-[#1B2B4B] transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div className="flex items-center gap-2">
          <ClipboardList size={20} style={{ color: '#2AABBF' }} />
          <h1 className="text-2xl font-bold" style={{ color: '#1B2B4B' }}>Nueva indicación</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm space-y-5">
        ...
        <div>
          ...
          <select name="paciente_id" required style={RING} className={SELECT}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className={LABEL}>Tipo de indicación *</label>
            <select name="tipo" required style={RING} className={SELECT}>
          ...
          <div>
            <label className={LABEL}>Nombre / medicamento *</label>
            <input name="nombre" type="text" required
              placeholder="Ej: Metformina 500, Curación herida…"
              style={RING} className={INPUT} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="via" style={RING} className={SELECT}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="fecha_fin" type="date" style={RING} className={INPUT} />
          </div>
        </div>
        ...
        <div>
          ...
          <input name="responsable" type="text"
            placeholder="Nombre del médico que recetó"
            style={RING} className={INPUT} />
        </div>
