...
import { getBalancePaciente } from '@/lib/actions/finanzas'
...
export default async function BalancePacientePage({ params }: { params: { id: string } }) {
  let balance: Awaited<ReturnType<typeof getBalancePaciente>> | null = null
  try { balance = await getBalancePaciente(params.id) } catch { notFound() }
  if (!balance) notFound()

  const { paciente: p, incomes, expenses } = balance
  const positivo = balance.utilidad_estimada >= 0

  const activosI = incomes.filter(i => i.estatus !== 'cancelado')
  const canceladosI = incomes.filter(i => i.estatus === 'cancelado')
  const activosE = expenses.filter(e => e.estatus !== 'cancelado')
  const canceladosE = expenses.filter(e => e.estatus === 'cancelado')
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold" style={{ color: '#1B2B4B' }}>
            Balance · {p.nombre} {p.apellido}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">Balance financiero completo del paciente</p>
        </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm">
        <h3 className="font-semibold text-sm uppercase tracking-wide text-gray-400 mb-5">Resumen financiero</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="text-center">
            <p className="text-xs text-gray-400 mb-1">Ingresos confirmados</p>
            <p className="text-xl font-bold" style={{ color: '#059669' }}>${formatMonto(balance.ingresos_confirmados)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-400 mb-1">Por cobrar</p>
            <p className="text-xl font-bold" style={{ color: '#D97706' }}>${formatMonto(balance.total_por_cobrar)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-400 mb-1">Salidas pagadas</p>
            <p className="text-xl font-bold" style={{ color: '#DC2626' }}>${formatMonto(balance.salidas_pagadas)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-400 mb-1">Salidas pendientes</p>
            <p className="text-xl font-bold" style={{ color: '#EA580C' }}>${formatMonto(balance.salidas_pendientes)}</p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Ingresos confirmados − salidas pagadas</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold" style={{ color: positivo ? '#059669' : '#DC2626' }}>
                {positivo ? '' : '-'}${formatMonto(Math.abs(balance.utilidad_estimada))}
              </p>
              <p className="text-xs text-gray-400">
                Margen: {balance.margen_estimado.toFixed(1)}%
              </p>
            </div>
          </div>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold" style={{ color: '#1B2B4B' }}>
            Ingresos ({activosI.length})
          </h2>
          <Link href={`/finanzas/ingresos?paciente_id=${p.id}`}
            className="text-xs text-[#2AABBF] hover:underline">Ver todos</Link>
        </div>
        {activosI.length === 0 ? (
          <p className="text-sm text-gray-400 bg-white rounded-xl p-4 shadow-sm">Sin ingresos registrados</p>
        ) : (
              ...
              return (
                <Link key={ing.id} href={`/finanzas/ingresos/${ing.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{formatFecha(ing.fecha_pago)}</p>
                  </div>
                  <div className="text-right">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold" style={{ color: '#1B2B4B' }}>
            Salidas ({activosE.length})
          </h2>
          <Link href={`/finanzas/salidas?paciente_id=${p.id}`}
            className="text-xs text-[#2AABBF] hover:underline">Ver todas</Link>
        </div>
        {activosE.length === 0 ? (
          <p className="text-sm text-gray-400 bg-white rounded-xl p-4 shadow-sm">Sin salidas registradas</p>
        ) : (
          <div className="space-y-2">
            {activosE.map(sal => {
              ...
              return (
                <Link key={sal.id} href={`/finanzas/salidas/${sal.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{formatFecha(sal.fecha_salida)}</p>
                  </div>
      ...
      {/* Cancelados */}
      {(canceladosI.length > 0 || canceladosE.length > 0) && (
        <details className="bg-gray-50 rounded-xl p-4">
          <summary className="cursor-pointer text-xs font-medium text-gray-400">
            Movimientos cancelados ({canceladosI.length + canceladosE.length})
          </summary>
          <div className="mt-3 space-y-1">
            {[...canceladosI, ...canceladosE].map(mov => {
