...
export default async function CasoDetailPage({ params }: { params: { id: string } }) {
  ...
  let bitacora: Awaited<ReturnType<typeof getBitacoraPorCaso>> = []
  try { bitacora = await getBitacoraPorCaso(params.id) } catch { /* sin datos */ }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-5">
          <ScrollText size={15} style={{ color: '#2AABBF' }} />
          <h2 className="text-sm font-semibold" style={{ color: '#1B2B4B' }}>
            Bitácora ({bitacora.length})
          </h2>
        </div>
        {bitacora.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">Sin movimientos registrados aún</p>
        ) : (
          <BitacoraTimeline movimientos={bitacora} showCaso={false} />
        )}
      </div>
