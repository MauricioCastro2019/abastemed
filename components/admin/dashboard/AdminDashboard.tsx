...
async function getMetricas(): Promise<MetricasDashboard> {
  ...
  if (!supabaseUrl || supabaseUrl.includes('placeholder')) {
    return { casos_activos: 1, enfermeros_disponibles: 3, turnos_hoy: 2, cobranza_pendiente: 0 }
  }
  try {
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <a key={href} href={href}
                  ...
                  <Icon size={16} className="text-gray-400 group-hover:text-[#2AABBF] transition-colors" />
                  <span className="text-sm text-gray-600 group-hover:text-[#1B2B4B] font-medium">{label}</span>
                </a>
              ))}
            </div>
          </CardContent>
