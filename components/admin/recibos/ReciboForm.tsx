...
// ----------------------------------------------------------------
// Estilos reutilizables (consistentes con el resto de la app)
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
interface ItemRow {
  uid: string   // key local, no va a la DB
  descripcion: string
  cantidad: string
...
// ----------------------------------------------------------------
export function ReciboForm({ recibo }: Props) {
  ...
  // ---- Actualizar una celda de la fila y recalcular importe ----
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    formData.set('items', JSON.stringify(serialized))

    startTransition(async () => {
      try {
        if (recibo) {
          await actualizarRecibo(recibo.id, formData)
        } else {
          await crearRecibo(formData)
        }
      } catch (err: unknown) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <section className="bg-white rounded-xl p-6 shadow-sm">
        ...
        <textarea
          name="observaciones"
          defaultValue={recibo?.observaciones ?? ''}
          rows={3}
          className={INPUT}
          placeholder="Notas adicionales para el recibo (opcional)..."
        />
      </section>
